...
def getDataFromApi():
    for par in pares:
        ...
        if par in tablas:
                ...
                sql = "create table {}(time date,close double,open double,low double,high double,volume double,spread double);".format(par)
                mycursor.execute(sql)
                mydb.commit()
        ...
        while(desde < today):
            try:
                ...
                for i in z:
                    ...
                    if dt.datetime.fromtimestamp(float(i.time)).replace(hour=0,minute=0,second=0,microsecond=0) not in pd.to_datetime(prize_df.index).tolist(): 
